...
원인: 두 워커가 제한이 풀릴 때마다 같이 출발해 로그인 두 벌(4요청)이 창(5개)을 거의 다 쓰고, 그 뒤 페이지
요청은 하나만 200을 받아 그 작업만 나아갑니다(2026-09-27 정정: 처음에는 "다섯 번째 요청(한 페이지)을 먼저 보낸
작업만"이라고 썼는데, 위 로그에서 페이지는 네 번째 요청이고 나머지 페이지 요청은 403·429를 받았습니다). 작업마다 세면 "창이 로그인 비용보다 크면 주기마다 나아간다"는

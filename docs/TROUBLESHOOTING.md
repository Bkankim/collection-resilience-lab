...
- **수정**: 코드는 바꾸지 않았다. limiter를 환경변수(`WORKER_LIMIT_MAX`, `WORKER_LIMIT_DURATION_MS`)로 "N ÷ 작업당 요청 수" 개 / W 초로 조이면 풀린다. 기본값을 거기에 맞추지 않은 이유는 속도 제한이 꺼진 기준선에서 200건에 30분이 넘게 걸려서다(`process.ts` `DEFAULT_WORKER_LIMITER` 주석, 커밋 4f1392c Rejected).

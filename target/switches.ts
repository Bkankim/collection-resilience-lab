 ...
 * 못한다.
 *
 * **출발지 차단은 속도 제한에 종속이다.** 차단으로 넘어가는 조건이 "429를 M회
 * 누적"이라서, 속도 제한이 꺼져 있으면 429가 안 나고 따라서 차단도 안 난다.
 * `{ ipBlock: true, rateLimit: false }`는 설정으로는 받아들여지지만 아무 일도
 * 일어나지 않는다. 측정 시나리오를 짤 때 둘을 같이 켜야 한다.
 */
...
export type Thresholds = {
   ...
   *
   * **여기에는 기간이 없다.** W는 창이 있는데 M은 차단 해제나 설정 변경 전까지
   * 계속 쌓인다. 실제 임계치에는 보통 기간이 붙지만("10분 안에 M회"), 측정
   * 단위를 하나 줄이려고 뺐다. 결과표에 M을 적을 때 이 점을 같이 적는다.
   */
  blockAfter: number;

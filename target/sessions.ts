...
export class SessionStore {
   ...
   *
   * 지금은 `/admin/reset` 전까지 계속 쌓인다. 상한을 둔다면 기준은 쿠키
   * `Max-Age`다. 규격을 지키는 클라이언트는 그 뒤로 쿠키를 안 보내므로 마커도
   * 그만큼만 살면 된다. 실험실 규모에서는 무해해서 안 넣었다.
   */
   ...
   * 지점이 두 군데이기 때문이다.
   *
   * 새로 발급하므로 `issuedAt`도 새로 찍힌다. 즉 세션 만료 임계값 `S`는 로그인
   * 시각이 아니라 **2차 인증 통과 시각**부터 잰다. 실제 기관은 여기에 유휴
   * 만료(무활동 N분)를 더 얹는 경우가 많지만, 여기서는 절대 만료 하나만 쓴다.
   */

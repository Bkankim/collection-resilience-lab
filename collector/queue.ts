 ...
 *
 * **수명(#19).** 워커는 페이지를 받을 때마다 이 해시에 쓰므로, 끝나지 않은 작업(대기·실패)에도 부분 결과가
 * 있다. 실패 작업의 부분 결과는 지우지 않는다. 사람이 그 작업을 다시 돌리면(`job.retry()`) 체크포인트부터
 * 이어받는 데 쓰기 때문이다. API는 completed일 때만 읽으므로 부분 결과가 완료로 보이지 않는다. **작업
 * 상태를 보지 않고 이 키를 직접 읽지 않는다.** 1페이지부터 시작하는 실행은 이 키를 먼저 지우고, 이어받는
 * 실행은 체크포인트의 행 수보다 적으면 1페이지부터 다시 받는다(`worker/process.ts`). 운영 규칙: 작업을
 * 지울 때 이 키도 같이 지운다. 남겨 두면 만료 없이 Redis에 쌓인다(같은 ID로 다시 넣으면 그 실행이 지운다).
 */

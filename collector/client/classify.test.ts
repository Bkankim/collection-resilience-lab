...
import type { Classification, ClassifyInput, Headers, RawResponse } from './classify.js';
...
/** 판정 결과를 모아 두었다가 마지막에 "원본을 남겨야 하는 종류는 원본을 실었나"를 한 번에 본다. */
const seen: Classification[] = [];
function run(input: ClassifyInput): Classification {
...
describe('응답 분류기: 속도 제한', () => {
  it('429 + Retry-After는 그 값을 대기 시간으로 쓴다', () => {
    const result = run(json(429, { error: 'RATE_LIMITED' }, { 'retry-after': '7' }));
  ...
  it('Retry-After가 날짜 형식이면 해석하지 않고 기본 대기를 쓴다', () => {
    const result = run(json(429, {}, { 'retry-after': 'Wed, 23 Sep 2026 03:00:00 GMT' }));
    expect(result).toMatchObject({
      kind: 'RATE_LIMITED',
      retryAfterSec: DEFAULT_RATE_LIMIT_WAIT_SEC,
...
describe('응답 분류기: 401은 헤더로 갈린다', () => {
  it('401 + X-Session-Expired는 세션 만료다', () => {
    const result = run(json(401, { error: 'SESSION_EXPIRED' }, { 'x-session-expired': '1' }));
    expect(result).toMatchObject({ kind: 'SESSION_EXPIRED' });
  });

  it('401 + X-Auth-Failed는 자격증명 실패다', () => {
    const result = run(json(401, { error: 'BAD_CREDENTIALS' }, { 'x-auth-failed': '1' }));
  ...
  it('두 헤더가 함께 오면 자격증명 실패로 읽는다', () => {
    ...
    const result = run(json(401, {}, { 'x-session-expired': '1', 'x-auth-failed': '1' }));
    expect(result).toMatchObject({ kind: 'AUTH_FAILED' });
  });

  it('헤더 이름의 대소문자와 값에 상관없이 X-Auth-Failed가 있으면 자격증명 실패다', () => {
    expect(run(json(401, { error: 'LOCKED' }, { 'X-Auth-Failed': '1' }))).toMatchObject({ kind: 'AUTH_FAILED' });
    expect(run(json(401, {}, { 'x-auth-failed': '0' }))).toMatchObject({ kind: 'AUTH_FAILED' });
  });

  it('헤더 없는 401 + NO_SESSION은 인증부터 하면 되므로 세션 만료로 묶는다', () => {
    const result = run(json(401, { error: 'NO_SESSION' }));
    expect(result).toMatchObject({ kind: 'SESSION_EXPIRED' });
    expect(FIRST_REMEDY.SESSION_EXPIRED).toEqual(['REAUTH']);
  });

  it('헤더도 알려진 본문도 없는 401은 추측하지 않고 UNKNOWN이다', () => {
    const result = run({ status: 401, headers: {}, body: Buffer.from('<html>unauthorized</html>') });
    expect(result).toMatchObject({ kind: 'UNKNOWN' });
    expect(FIRST_REMEDY.UNKNOWN).not.toContain('REAUTH');
  });
});

describe('응답 분류기: 403은 차단과 우리 흐름 오류를 가른다', () => {
  it('403 + Retry-After는 출발지 차단이다', () => {
    const result = run(json(403, { error: 'IP_BLOCKED', origin: '127.0.0.1', retryAfterSec: 30 }, { 'retry-after': '30' }));
  ...
  it('2차 인증 전 조회(OTP_REQUIRED)는 세션 만료가 아니라 UNKNOWN이다', () => {
    ...
    const result = run(json(403, { error: 'OTP_REQUIRED' }));
  ...
  it('계좌 불일치(ACCOUNT_MISMATCH)도 UNKNOWN이다', () => {
    expect(run(json(403, { error: 'ACCOUNT_MISMATCH' }))).toMatchObject({ kind: 'UNKNOWN' });
  });

  it('2차 인증 코드 거부(OTP_REJECTED)는 다음 코드로 다시 하면 되는 일시 실패다', () => {
    expect(run(json(403, { error: 'OTP_REJECTED' }))).toMatchObject({ kind: 'TRANSIENT' });
  });

  it('본문이 IP_BLOCKED라도 Retry-After가 없으면 차단으로 단정하지 않는다', () => {
    // 대상 서버는 차단에 항상 Retry-After를 싣는다. 없으면 계약이 깨진 응답이다.
    expect(run(json(403, { error: 'IP_BLOCKED' }))).toMatchObject({ kind: 'UNKNOWN' });
  });
});

describe('응답 분류기: 일시 장애', () => {
  it.each([500, 502, 503, 504])('HTTP %i는 TRANSIENT다', (status) => {
    expect(run({ status, headers: {}, body: Buffer.from('bad gateway') })).toMatchObject({ kind: 'TRANSIENT' });
  });

  it('503 + Retry-After는 그 값을 싣는다', () => {
    expect(run({ status: 503, headers: { 'retry-after': '5' }, body: Buffer.alloc(0) })).toMatchObject({
      kind: 'TRANSIENT',
      retryAfterSec: 5,
    });
  });
  ...
  it.each([
    ...
    expect(run(network(error))).toMatchObject({ kind: 'TRANSIENT' });
  });

  it('DNS 실패와 사용자가 끊은 요청은 UNKNOWN이다', () => {
    expect(run(network({ code: 'ENOTFOUND', message: 'getaddrinfo ENOTFOUND target' }))).toMatchObject({ kind: 'UNKNOWN' });
    expect(run(network({ name: 'AbortError', message: 'This operation was aborted' }))).toMatchObject({ kind: 'UNKNOWN' });
  });
});

describe('응답 분류기: 200', () => {
  it('정상 거래 페이지는 성공이고 EUC-KR 한글 적요를 읽는다', () => {
    const result = run(html(1));
  ...
  it('요약 줄은 있으나 표가 없으면 PARSE_FAILED다', () => {
    ...
    const result = run({ status: 200, headers: { 'content-type': TRANSACTIONS_CONTENT_TYPE }, body: encodeEucKr(text) });
  ...
  it('200 JSON 응답은 PARSE_FAILED다', () => {
    expect(run({ ...json(200, { ok: true }), headers: {} })).toMatchObject({ kind: 'PARSE_FAILED' });
  });

  it('charset이 EUC-KR이 아니라고 선언되면 파싱하지 않는다', () => {
    const result = run({ ...html(1), headers: { 'content-type': 'text/html; charset=utf-8' } });
...
describe('응답 분류기: 그 밖', () => {
  it('400 BAD_PAGE는 우리 요청이 틀린 것이라 UNKNOWN이고 원본을 싣는다', () => {
    ...
    expect(run(input)).toMatchObject({ kind: 'UNKNOWN', raw: input, detail: expect.stringContaining('BAD_PAGE') });
  });

  it('JSON이 아닌 본문에서도 예외를 던지지 않는다', () => {
    const garbage = Buffer.from([0xff, 0xfe, 0x7b, 0x00, 0x22]);
    for (const status of [400, 401, 403, 418]) {
      expect(() => run({ status, headers: {}, body: garbage })).not.toThrow();
    }
  });

  it('원본을 남기는 대응을 가진 모든 판정에 원본이 실려 있다', () => {
    // 위 테스트들이 만든 판정 전부를 본다. CAPTURE_RAW인데 raw가 없으면 사람이 볼 것이 없다.
    const failures = seen.filter((result) => !result.ok);
    ...
    for (const result of failures) {
      ...
      expect('raw' in result && result.raw !== undefined, `${result.kind}: ${result.detail}`).toBe(captures);
    }
  });
});
